...
//
// DEBUG: temporary [greetListener] console.log lines added below to trace
// exactly where a join/leave event dies. Remove once confirmed working.
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleEvent(sock, event) {
  ...
  // DEBUG
  console.log("[greetListener] event fired:", { from, action, participants })
  ...
  for (const jid of participants) {
    ...
    console.log("[greetListener] sending for:", jid, "caption:", caption)
...
function register(sock) {
  if (_registeredSockets.has(sock)) {
    console.log("[greetListener] register() called again for same sock — skipping duplicate")
    return
  }
