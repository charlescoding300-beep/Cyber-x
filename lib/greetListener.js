...
async function getProfilePicOrFallback(sock, jid, fallbackUrl) {
  try {
    ...
    if (url) return url
    console.log(`[greetListener] profilePictureUrl returned empty for ${jid} — using fallback`)
  } catch (err) {
    console.log(`[greetListener] profilePictureUrl FAILED for ${jid}: ${err.message} — using fallback`)
  }
  return fallbackUrl
}
...
async function handleGreetEvent(sock, event) {
  ...
  for (const jid of participants) {
    ...
    const mention = `@${name}`
    const bio     = await getBio(sock, jid)
    ...
    const caption = grayBold(rawCaption)
