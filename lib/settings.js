...
// Lazy, best-effort — if Redis isn't configured or the module fails to
// load for any reason, settings still work purely from local disk exactly
// as before. This is additive, never load-bearing for correctness.
let settingsBackup = null
try { settingsBackup = require("./settingsBackup") } catch {}
...
// Fire-and-forget Redis push — never blocks or throws into the caller.
// This is what makes a session's settings survive even a fresh disk
// (a redeploy, a wiped container) rather than just a plain restart,
// and it's what gets deleted the moment that session logs out for real.
function pushToRedisBestEffort(phone, data) {
  if (!settingsBackup) return
  settingsBackup.pushSettings(phone, data).catch(() => {})
}
...
function writeUserFile(phone, data) {
  ...
  const t = setTimeout(() => {
    try {
      ...
      pushToRedisBestEffort(phone, data)
    } catch (e) {
...
function forUser(phoneOrJid) {
  ...
  return {
    ...
    // Every write now flushes immediately AND pushes to Redis — no more
    // 300ms window where a quick restart can lose the change. The old
    // debounced writeUserFile path still exists below for anything that
    // doesn't explicitly call .flush(), but .set() itself is now instant.
    set(keyOrObj, value) {
      const fileData = load()
