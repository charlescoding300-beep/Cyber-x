...
// ─────────────────────────────────────────────────────────────────────────────
// FIX: the original redisSet() posted a single flat command array
// (["SET", key, value]) to Upstash's /pipeline endpoint. /pipeline
// expects an ARRAY OF command arrays (e.g. [["SET", key, value]]) because
// its whole purpose is running multiple commands in one request — sending
// it a flat single command is a malformed payload, so Upstash returned a
// non-200 response every time. That's why every push silently failed:
// res.statusCode === 200 was never true, and the actual response body
// (which would have explained why) was discarded via res.resume()
// without ever being read.
//
// Fix: post to the ROOT URL (not /pipeline) with the single flat command
// array — this is the correct format for Upstash's regular single-command
// REST API, same pattern lib/sessionBackup.js already uses correctly.
// Also: read and log the actual response body on any non-success result,
// so future failures show a real reason instead of a blank "Push failed".
// ─────────────────────────────────────────────────────────────────────────────

async function redisSet(key, value) {
  if (!REDIS_URL || !REDIS_TOKEN) return { ok: false, error: "Redis not configured" }
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request({
      ...
      path: url.pathname || "/",   // root, not /pipeline — see note above
      method: "POST",
      headers: {
      ...
      res.on("end", () => {
        if (res.statusCode === 200) {
          ...
          resolve({ ok: false, error: `HTTP ${res.statusCode}: ${chunks.slice(0, 300)}` })
        }
      })
    })
...
async function redisGet(key) {
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request({
      ...
      res.on("data", c => d += c)
      res.on("end", () => {
        try { resolve(JSON.parse(d)?.result || null) } catch { resolve(null) }
      })
    })
...
async function pushAllData() {
  ...
  try {
    ...
    const result = await redisSet(REDIS_KEY, JSON.stringify(snapshot))
    if (result.ok) {
      console.log(`[PERSIST] ✅ Pushed ${keys.length} file(s) to Redis`)
    } else {
      console.error(`[PERSIST] ✗ Push failed: ${result.error}`)
    }
    return result.ok
